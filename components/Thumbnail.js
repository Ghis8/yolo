...
const Thumbnail=forwardRef(({result},ref)=>{
  ...
  return (
    <div ref={ref} className="group cursor-pointer p-2 transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50">
      <Image src={`${BASE_URL}${result.backdrop_path || result.poster_path}` || `${BASE_URL}${result.poster_path}`} layout='responsive' height={1080} width={1920} alt="moviesImg" />
      <div className="p-2">
        ...
        <p className="flex items-center opacity-0 group-hover:opacity-100">
            ...
            <MdThumbUpOffAlt className="h-5 mx-2"/> {result.vote_count}
        </p>
      </div>
    </div>
  )
